...
if (&Time2msec($rec_time[0]) == &Time2msec($margin_time)) {
    ...
    # 先頭を抜き取る(marginより手前なので無視)
    shift(@rec_time);
    # tmpがmarginを超えるまで、tmpに先頭を抜き取り続ける
    while (my $tmp = shift @rec_time) {
        # tmp と marginを比較
        if (&Time2msec($margin_time) < &Time2msec($tmp)) {
            # 配列総数が奇数のときは margin tmp rectime[n] の順で戻す
            unless ($#rec_time % 2) {
            unshift(@rec_time, $tmp);
            unshift(@rec_time, $margin_time);
            } else {
            # 配列総数が偶数のときは tmp rectime[n] の順で戻す
            unshift(@rec_time, $tmp);
            }
            last;
        }
